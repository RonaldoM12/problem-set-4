...
def similar_actors_genre(data, query_id, top_10 = 10):
    
    data = pd.read_json("imdb_movies.json")

    names = data["name"]

    feature = data.drop(columns=["name"])

    query_id = "nm1165110" #Chris Hemsworth ID number

    query = feature.loc[query_id].values.reshape(1, -1)

    euclidean = DistanceMetric.get_metric("Euclidean")

    distances = euclidean.pairwise(query, feature.values)[0]

    result = pd.DataFrame({
        "actor_id": feature.index,
        "name": names,
        "euclidean_dist": distances
    }).sort_values(by="euclidean_dist")
